...
2. **Verify and fold the touched chunks.** Given `startOffset` (public input) and `ownStreamBytes` (step 1), walk the declared heterogeneous chunks and require each to contain bytes owned by this proof. For each chunk `k ∈ [1, T]`: for a blob, slice the bytes that remain after its boundary prefix/suffix (all `chunkSize` bytes for an interior blob), combine them with `opaquePrefixBytes` / `opaqueSuffixBytes` — relevant only for `k = 1` / `k = T` respectively — and reconstruct exactly `chunkSize` bytes before running the KZG check. A calldata chunk instead resolves its complete whole-segment extent by matching its anchored hash at candidate segment-end offsets (§3.1). After the hash-matched walk consumes exactly `ownStreamBytes`, derive `endOffset`: emit `0` when the terminal calldata or blob chunk is fully consumed; emit the positive number of bytes consumed within the terminal blob only when `opaqueSuffixBytes` proves that blob is shared. Fold the dataRollingHash:
   ```
