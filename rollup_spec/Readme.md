...
Each rollup proof here covers `N ≥ 1` consecutive conflations (`N = 3` in both, illustratively). Chunks transporting the underlying byte stream are a separate, independently-sized partition: a rollup proof's conflations may span multiple chunks, and several rollup proofs' conflations may share a blob chunk, with no fixed alignment between the two (§3.1). The rollup-aggregation step is flat across the `M` rollup proofs; hierarchical (k-ary tree) aggregation is a future option, not part of this iteration.
...
The DA payload must contain the exact inputs required to re-execute the L2 blocks from the previous finalized state. Because the zk-proof guarantees transition validity, any data that is a deterministic *output* of execution can be stripped.
...
**Encoding and compression:** Each conflation's truncated-block payload is compressed independently as one self-delimiting zstd frame; the resulting segments are concatenated in block order into the continuous DA stream (§3.1). The rollup guest receives each frame as witness data, requires it to consume all witnessed frame bytes, decompresses it independently, and matches the result against the canonical truncated-block RLP; it does not need to reproduce the compressor's choices. The stream is then sliced into chunks for transport; a blob chunk this proof touches may be only partially filled by its own data at either end, with the remainder witnessed as opaque bytes (§2.2). Using a standard, unconstrained compressor significantly increases the effective throughput per chunk compared to the current LZSS-based approach, without making the guest pay the cost of recompression or require byte-for-byte compressor determinism.
