...
The same 20-field tuple as the rollup-aggregation proof (§2.4). `(parentDataRollingHash, startOffset)` is this proof's start stream position; `(endDataRollingHash, endOffset)` is its end stream position (§3.1) — `startOffset` is a request input, while `endOffset` is derived from the total length of the witnessed length-prefixed segments in the stream range.
...
**Encoding and compression:** Each conflation's truncated-block payload is compressed independently with zstd and prefixed with its compressed length; the resulting segments are published as the continuous DA stream (§3.1). The rollup guest receives each length-prefixed segment as witness data, decompresses it independently, and matches the result against the canonical truncated-block RLP; it does not need to reproduce the compressor's choices. The stream is then sliced into fixed-size chunks for transport; a chunk this proof touches may be only partially filled by its own data at either end, with the remainder witnessed as opaque bytes (§2.2). Using an unconstrained standard compressor improves throughput without making the guest pay the cost of recompression or require byte-for-byte compressor determinism.
...
**Binding to the compression proof.** Identical to §5.1: for each calldata submission the prover produces a compression proof attesting to (a) correct decompression of each length-prefixed segment covered by `compressedData` and its match against the canonical truncated-block encoding, and (b) the polynomial evaluation against `keccak256(compressedData)`, and the proof aggregates its related l2-execution proofs and emits the unified 20-field public-input tuple. The L1 contract only anchors the dataRollingHash at submission time; verification happens at finalization (§5).
