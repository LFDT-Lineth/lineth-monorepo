...
- The environment-dependent `isAllowedCircuitID` bitmask gating which inner circuit/program identities step 1 may accept (bit *i*, LSb→MSb, allows circuit ID *i*). It mirrors the prover's `Aggregation.is_allowed_circuit_id` config and is a proving-policy input only — not one of the 16 public-input fields and not part of `dynamicChainConfigHash`. This bitmask is an off-chain, prove-time convenience filter only; the authoritative, version-specific, **on-chain-anchored** notion of program identity is `programVk` (above) checked against L1's `approvedVks` set (§2.6 *Guest Program Anchoring (ProgramVK)*, §5). `isAllowedCircuitID` should track the same approval policy locally so a prover fails fast on an unapproved inner proof instead of only discovering the rejection at L1 finalization.

**Statement (RISC-V Guest)**

1. **Verify** all `M` inner proofs cryptographically against their claimed public inputs, using each proof's `programVk_i` (private input, above) as the recursive verifier's verifying key, and accepting an inner proof only if its circuit/program identity is permitted by the `isAllowedCircuitID` bitmask.
...
| **ProgramVK anchoring** | Guest identity is gated only off-chain, at prove time, via the `isAllowedCircuitID` bitmask (§2.3) — equivalently, trust in "the deployed verifier bytecode is the right circuit"; nothing on L1 stops an operator swapping in an unapproved guest/circuit and still finalizing | Exec and rollup guests each commit a `programVk`, bubbled up as a single combined PI set field `programVks` (§2.2–§2.4; a canonical distinct, sorted-ascending array; exec vs rollup not distinguished — internal guest bookkeeping); L1 runs an order-independent set-membership check of every entry against a combined `approvedVks` set at finalization (§2.6, §5) and reverts otherwise; council-managed add/remove; aggregation-grained, so multiple approved exec VKs (i.e. different forks) can finalize together in one call |
