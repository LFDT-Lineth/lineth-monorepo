...
**A second migration is needed for the dataRollingHash transition.** This section describes the already-specified state-root-hash → 3-argument-shnarf bridge. Moving from that 3-argument shnarf (`Hash(parentShnarf, lastBlockHash, blobHash)`) to the 2-argument dataRollingHash (`Hash(parentDataRollingHash, chunkHash)`, §3.1) is a second, analogous transition. It is implemented by the `reinitializeLineaRollupV11(bytes32 _currentFinalizedShnarf)` bridge on `LinethRollup` with the following path-selection rule:

- **Fresh-genesis networks** (new testnets / local / CI): `initialize` seeds the genesis DA stream position directly — `genesisDataRollingHash = EMPTY_HASH` (no chunks folded), `genesisOffset = 0` (fresh-start sentinel) — and stores `currentFinalizedPositionCommitment = keccak256(genesisDataRollingHash || encodeOffset(0))`. The genesis dataRollingHash is anchored so the first submission can chain from it. No legacy bridge runs.
- **Live chains (in-place upgrade):** the bridge reinterprets the slot that held the plain finalized shnarf as the previous end dataRollingHash and **reseals** it as the initial position commitment with offset `0`: `currentFinalizedPositionCommitment = keccak256(bridgedShnarf || encodeOffset(0))`. The bridged value is also anchored into the dataRollingHash membership set so post-upgrade submissions chain from it. The caller must supply the exact current `currentFinalizedShnarf` value; the bridge reverts (`BridgedShnarfMismatch`) if live state has drifted, and the value is captured before being overwritten so the bridge is one-way. The first post-upgrade finalization then supplies `(prevDataRollingHash = bridgedShnarf, prevOffset = 0)` and takes the fresh-start branch (`startOffset == 0`) of the position-continuity disjunction (§5). This is the path-selection rule that was previously left TBA.
