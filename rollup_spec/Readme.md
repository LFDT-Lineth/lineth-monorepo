...
| `chunks` | One `{chunkHash, isCalldata}` entry per touched chunk `k ∈ [1, T]`. `chunkHash` is the binding hash anchored on L1 for that chunk; `isCalldata` (default `false`) selects the in-guest check (§3.1): a blob chunk is cross-checked against `kzg_commitment_to_versioned_hash(computedChunkCommitment_k)` (`0x01 ‖ sha256(kzgCommitment)[1:]`), a calldata chunk against `keccak256(fullChunkBytes_k)`, both recomputed by the guest directly from the reconstructed chunk bytes (no separate KZG opening proof is witnessed or verified — the commitment's binding property already ties the anchored hash to the exact bytes) |
...
The two kinds differ in whether they can be **shared** across a proof-range boundary. A blob chunk is fixed-size, so a range may begin or end mid-blob, sharing that blob with a neighbouring rollup proof (or, at a finalization boundary, with a fresh-start tail — §5); the neighbour's bytes are witnessed as opaque prefix/suffix bytes to reconstruct the full blob. A calldata chunk is **range-aligned**: it packs a whole number of conflation segments and shares no bytes with a neighbouring proof, so it is wholly owned by exactly one proof — never shared, never reconstructed from opaque bytes, and never entered at a non-zero `startOffset`.

**Chunk-kind dispatch is explicit.** Each `chunks` entry carries an `isCalldata` flag (default `false`) that selects the in-guest check: a blob chunk is verified by computing its KZG commitment in-guest, versioned-hashing it (`0x01 ‖ sha256(kzgCommitment)[1:]`), and comparing against `chunkHash`; a calldata chunk is verified by `keccak256` of the reconstructed bytes, compared against `chunkHash`. The flag is witness data — the anchored `chunkHash` records the binding, not the kind — and it needs no independent L1 verification, because both arms terminate in `recomputed == chunkHash`: a flag that mismatches the chunk's actual construction fails the hash check. The flag selects which recomputation runs; the anchored hash carries the soundness. The rollup-aggregation request only ever sees the folded stream positions (§2.4), never raw chunks.

The two kinds differ in size, sharing, and how the guest locates their boundaries. A blob chunk is always `chunkSize` bytes, so the guest reconstructs it from its own slice plus the opaque prefix/suffix bytes when it is shared (§3.1). A calldata chunk packs a **whole number of conflation segments** — complete segments, no partial tail — and is range-aligned: it shares no bytes with a neighbouring proof, so it carries no opaque bytes and sits only at `startOffset = 0`. Its end therefore falls only at a segment boundary, and the guest resolves it by hashing the stream from the chunk's start to each candidate segment-end offset and accepting the one whose keccak256 equals the anchored `chunkHash`. The matching partition is itself the proof of the chunk's extent, so no length is witnessed, and a run of consecutive calldata chunks self-delimits. The guest asserts these per-kind invariants, so a declared-calldata chunk commits to the range-aligned, whole-segment shape and a declared-blob chunk to the fixed-size shape.
