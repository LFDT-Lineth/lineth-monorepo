...
2. **Use industry-standard primitives.** SNARK-friendly workarounds (MiMC hashing, custom LZSS) are replaced with standard algorithms (Keccak256, zstd, BLS12-381) that can be compiled directly into the guest.
...
| `chunks` | One entry per touched chunk `k ∈ [1, T]` — `{chunkHash, length?}`. `chunkHash` is the binding hash anchored on L1 for that chunk; its leading byte selects the in-guest check (§3.1): a blob chunk (`0x01 ‖ sha256(kzgCommitment)[1:]`) is cross-checked against `kzg_commitment_to_versioned_hash(computedChunkCommitment_k)`, a calldata chunk against `keccak256(fullChunkBytes_k)`, both recomputed by the guest directly from the reconstructed chunk bytes (no separate KZG opening proof is witnessed or verified — the commitment's binding property already ties the anchored hash to the exact bytes). `length` is present only for a calldata chunk: the exact `_compressedData` length the L1 contract hashed (a blob chunk is always `chunkSize`, so it carries no `length`). |
...
2. **Verify and fold the touched chunks.** Given `startOffset` (public input) and `ownStreamBytes` (step 1), walk the `T` witnessed chunks in stream order, advancing a byte cursor over `ownStreamBytes` by each chunk's size; `T` and `endOffset` follow from that walk rather than from a fixed `chunkSize` (chunks are heterogeneous — §3.1). For each chunk `k ∈ [1, T]`, dispatched on its kind (§3.1):
   - **Blob chunk** (fixed `chunkSize`): slice the corresponding portion of `ownStreamBytes` (all of it for an interior chunk; `chunkSize - startOffset` bytes for the first chunk if `k = 1`; up to `endOffset` bytes for the last chunk if `k = T`), combine it with the witnessed `opaquePrefixBytes` / `opaqueSuffixBytes` — relevant only for `k = 1` / `k = T` respectively, never for interior chunks — to reconstruct the chunk's full `chunkSize` bytes, and run the KZG commitment check.
   - **Calldata chunk** (variable `length_k`, range-aligned): assert no opaque prefix/suffix bytes attach to it (and, if `k = 1`, that `startOffset = 0`), slice exactly `length_k` of this proof's *own* stream bytes, and run the `keccak256` check.

   `endOffset` is the number of bytes consumed of the LAST chunk (a blob's `chunkSize` less its opaque suffix, or a calldata chunk's full `length`). Fold the dataRollingHash:
   ```
...
The two kinds differ in whether they can be **shared** across a proof-range boundary. A blob chunk is fixed-size, so a range may begin or end mid-blob, sharing that blob with a neighbouring rollup proof (or, at a finalization boundary, with a fresh-start tail — §5); the neighbour's bytes are witnessed as opaque prefix/suffix bytes to reconstruct the full blob. A calldata chunk is **range-aligned**: the submitter aligns each calldata submission with a finalization range, so its byte boundaries coincide with proof-range boundaries and it is wholly owned by exactly one proof — never shared, never reconstructed from opaque bytes, and never entered at a non-zero `startOffset`.

**Chunk-kind dispatch is self-describing.** The anchored `chunkHash` already records which binding applies, so the guest reads it off the hash rather than trusting a witness flag: a blob chunk's versioned hash is `0x01 ‖ sha256(kzgCommitment)[1:]` — its leading byte is always `0x01` — while a calldata chunk's hash is a bare `keccak256(compressedData)`. A chunk whose `chunkHash` leads with `0x01` is verified as a blob (KZG commitment computed in-guest, versioned-hashed, compared); any other leading byte is verified as calldata (`keccak256` of the reconstructed bytes, compared). This needs no per-chunk kind witness and adds nothing to the rollup-aggregation request, which only ever sees the folded stream positions (§2.4), never raw chunks. A calldata payload whose `keccak256` happens to lead with `0x01` (probability ~1/256) would be misrouted to the KZG check and fail — a liveness footnote, not a soundness hole: such a payload simply is not submittable in this system and the operator resubmits different bytes.

The two kinds carry different witness shapes. A blob chunk is always `chunkSize` bytes, so the guest needs no length to reconstruct it — only the opaque prefix/suffix bytes when it is shared (§3.1). A calldata chunk is variable-length and range-aligned, so its witness entry carries the exact `_compressedData` length and the guest slices precisely that many of its *own* stream bytes — a calldata chunk never carries opaque prefix/suffix bytes and is never entered at a non-zero `startOffset` (it is wholly owned by this proof). The witnessed length is not trusted: the guest rejects unless the keccak256 of exactly those bytes equals the anchored `chunkHash`.
...
A **stream position** is the pair `(R, c)`: `R` is the dataRollingHash after folding every chunk up to and including the one containing the position; `c` is the number of bytes consumed of that last-folded chunk — `c ∈ (0, chunkSize]` for a blob chunk (`c = chunkSize` means "exactly at this blob's end"), or `c ∈ (0, length]` for a calldata chunk, which is always consumed whole at a range boundary (§3.1). Excluding `c = 0` from this range gives every position a unique encoding; `c = 0` is reserved as the **fresh-start sentinel** used only at finalization boundaries (§5). Because the KZG polynomial evaluation is proven inside the zkVM, the evaluation point `X` and claim `Y` never appear on-chain — the L1 contract only checks `chunkHash` against the transaction's `VERSIONED_HASH` (or `keccak256(compressedData)` on the calldata path).
...
**Encoding and compression:** Each conflation's truncated-block payload is compressed independently with **zstd** and prefixed with its compressed length; the resulting segments are concatenated in block order into the continuous DA stream (§3.1). The compression profile is pinned byte-for-byte so the rollup guest reproduces the sequencer's exact bytes (required for the KZG check, §2.2): the zstd reference C library at **version 1.5.6**, one-shot `ZSTD_compress` at the default level **3**, **no dictionary**, a single frame with the **content-size flag set** and the **content checksum disabled**. zstd's output is deterministic for a fixed library version and level, so pinning both — on the sequencer, in this reference (`zstandard`), and in the guest's vendored source — makes the segment bytes reproducible outside the guest. zstd back-references never cross a segment boundary, so a rollup proof can recompress its own conflations without any foreign witness data. The stream is then sliced into fixed-size chunks for transport; a chunk this proof touches may be only partially filled by its own data at either end, with the remainder witnessed as opaque bytes (§2.2). Stripping the above outputs and using an unconstrained compressor significantly increases the effective throughput per chunk compared to the current LZSS-based approach.
