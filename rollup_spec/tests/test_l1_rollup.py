...
# The previously-finalized end position, passed to `finalize_rollup` as calldata
# and sealed in the state's position commitment (§3.6, enforced variant).
_PREV_OFFSET = 0
_END_OFFSET = 131072
...
def _base_state(approved_vks) -> LinethRollupState:
    ...
    return LinethRollupState(
        current_finalized_position_commitment=_position_commitment(
