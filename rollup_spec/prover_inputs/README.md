...
The `testdata/` fixtures are the **language-neutral contract**; `../proof_io_v1.py` is the wire authority and the guest dataclasses are the logical model — there is no separate JSON Schema. **Guest output vs prover output:** a guest emits its public-input tuple plus the revealed hash preimages; the `proof` bytes are attached by the zkVM/prover layer, not the guest, and are placeholders (`0x`) in the fixtures. The next proving step (or L1) consumes the prover output (guest output + `proof`).
