// Package abicheck keeps verifier-ray in step with the prover-ray encoder it
// pins. Two halves: the layout numbers must agree (TestABIAgreement), and the
// image fixture the Zig tests read must match what the pinned encoder produces
...
// proofABIPath is verifier-ray's pinned layout, the authority the encoder
// encodes against.
const proofABIPath = "../../src/proof_abi.zig"

// TestABIAgreement checks the pinned encoder's size and offset constants against
// the assertions in src/proof_abi.zig.
//
...
func TestABIAgreement(t *testing.T) {
	...
	require.NoError(t, err, "reading %s", proofABIPath)

	// wantSize maps a Zig type as written in proof_abi.zig to the encoder's
	...
	// wantOffset maps a pinned (type, field) to the encoder's offset constant.
	...
	// These must reference the encoder's constants, not literal copies of the
	...
	for _, m := range sizes {
		...
		require.True(t, ok, "%s pins a size for %q that this check does not map to a "+
	...
	for _, m := range fields {
		...
		require.True(t, ok, "%s pins an offset for %s.%s that this check does not map; "+
	...
	for _, m := range tags {
		...
		want := mustAtoi(t, m[3])
		got, ok := wantTag[key]
		require.True(t, ok, "%s pins a discriminant for %s.%s that this check does not map; "+
...
// verifierRayImagePath is the image test/proof_image_test.zig maps and reads.
// It is the only test in which a byte produced by the encoder is consumed by the
// actual verifier rather than by the encoder's own decoder.
const verifierRayImagePath = "../../testdata/proof_image.bin"
...
//
// The image is committed rather than generated at Zig test time so the Zig suite
// stays self-contained — it needs no Go toolchain. The cost is that the file can
// go stale, which is what this test prevents. Regenerate with
...
func TestVerifierRayImageIsUpToDate(t *testing.T) {
	...
	require.NoError(t, err, "reading %s", verifierRayImagePath)
