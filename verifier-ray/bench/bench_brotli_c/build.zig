...
pub fn build(b: *std.Build) void {
    ...
    // brotli's decode-only C sources, compiled for the same freestanding rv64im
    // target as the Zig guest. No libc is linked: vendor/shim supplies the
    // headers platform.h includes unconditionally. Unlike zstd there is no
    // static-context API -- BrotliDecoderCreateInstance takes alloc/free
    // callbacks -- so main.zig supplies a bump allocator over a fixed arena.
    main_mod.addIncludePath(b.path("vendor/brotli/include"));
    main_mod.addIncludePath(b.path("vendor/shim"));
    main_mod.addCSourceFiles(.{
        .root = b.path("vendor/brotli"),
        .files = &.{
            "common/constants.c",
            "common/context.c",
            "common/dictionary.c",
    ...
    common.installGuestElf(b, main_mod, "bench-brotli-c");
}
