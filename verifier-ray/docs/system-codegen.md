...
The same compiled system verifies proofs from every guest ELF in `codegen`'s `HonestRiscvGuests`, not just the one committed above — `main.zkc`'s circuit doesn't depend on the witness. `verifier-ray/codegen/generate-riscv-guest-proofs` proves every remaining guest and writes each one's real proof image to a gitignored scratch directory (`testdata/scratch/riscv-guest-proofs/`, regenerated by `make generate-testdata`), and `test/riscv_guest_proofs_test.zig` verifies every one of them through the real `verifier.verify()`. These aren't committed as testdata: at ~52MB per guest, committing all of them would be ~525MB of binary fixtures for no benefit over the one guest already committed.
