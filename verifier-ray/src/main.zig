...
fn runVerifier(input: *const verifier.ProofData) u8 {
...
fn loadNativeInput() *const verifier.ProofData {
    if (comptime !is_supported_native) {
        @compileError("native verifier libc path currently supports x86_64/aarch64 Linux and macOS only");
    }
    ...
    const mapped_addr = mmap(null, @sizeOf(verifier.ProofData), prot_read, map_private, fd, 0);
...
fn loadR5Input() *const verifier.ProofData {
    if (comptime !is_r5_zkvm) {
