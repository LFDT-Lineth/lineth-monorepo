...
/// contribution digest, one per limb.
...
/// the Poseidon2 Merkle-Damgård sponge digest over every committed round
/// preceding the message-bus coin round — the verifier-side counterpart to
/// prover-ray's `sharedRandomnessContribution` / `SharedRandomnessContributionChecker.Check`.
///
...
pub fn verify(comptime system: System, ctx: protocol.Context) Error!void {
    ...
    inline for (system.contribution_refs, 0..) |ref, i| {
        ...
        if (!claimed.eql(ext.Ext.lift(digest[i]))) return error.ContributionMismatch;
    }
}
