...
# Execute an already compiled ELF in fast mode (no tracing)
...
riscv-test zkc-exec JSON=path/to/input.json ZKC_MAIN=path/to/main.zkc ZKC_EXEC_FLAGS="--fast -v"
...
| `ZKC_EXEC_FLAGS`             | `--fast`                                                                               | Flags passed to `zkc exec` within `zkc-exec` and `elf-exec`                                                                                   |
...
`act4-exec` runs `zkc exec` through `zkc-exec` (so `ZKC_EXEC_FLAGS` applies) with stdout discarded; a test passes when zkc exits with code 0 and stderr is empty. A summary of ACT4 results is written in `results.txt`.

To rerun one generated ACT4 test through zkc:

```bash
zkc exec --fast act4/bin/logs/<test-name>.json ../../main/riscv/main.zkc
```
