...
/// zesu's `executor.executeStatelessInput` (same preamble: derives `pre_state_root`, builds the MPT
/// node index and block-hash table, validates the header chain) and its `executeBlockStateless`
/// body (same WitnessDatabase + Context wiring, same BAL validation), but returns
/// `ProofOutputWithLogs` — full receipts (with `.logs`) instead of the vanilla guest's bloom-only
/// projection.
...
fn executeBlockStatelessWithLogs(
    ...
    // Fail closed: an unresolvable fork name must reject the block, never silently fall back to a
    // spec derived from (attacker-controlled) block_number/timestamp.
