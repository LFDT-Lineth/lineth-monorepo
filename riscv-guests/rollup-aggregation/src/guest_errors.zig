//! Deterministic guest exit-code taxonomy for the rollup-aggregation guest.
//!
...
/// The 2-byte schema id is missing or doesn't match `rollup_aggregation_ssz.INPUT_SCHEMA_ID` — the
/// envelope itself is not recognizable, before any SSZ container is even attempted.
...
/// `rollup_proofs` decoded successfully but is empty — the aggregation guest needs at least one
/// embedded rollup proof to source its "first"/"last" echoed fields from.
...
/// Every error a rollup-aggregation guest function deliberately returns, in `exitCode`'s arm order.
pub const rollup_aggregation_errors = error{
    MalformedFrame,
    InvalidSsz,
    BoundsViolation,
    EmptyProofs,
