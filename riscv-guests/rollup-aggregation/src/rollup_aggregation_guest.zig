...
fn guestMain() callconv(.c) noreturn {
    ...
    const out = runRollupAggregationGuest(allocator, raw_input) catch |err| {
...
pub fn runRollupAggregationGuest(allocator: std.mem.Allocator, raw_input: []const u8) ![]const u8 {
    const decoded = try rollup_aggregation_ssz.decodeInput(allocator, raw_input);
    const result = try rollup_aggregation.run(allocator, decoded);
    return rollup_aggregation_ssz.encodeOutput(allocator, result) catch return error.OutputEncodeFailed;
}

comptime {
    // Export `main` only for the freestanding RISC-V guest, which owns its entry point. Native
    // builds import this module as a library (the host test calling `runRollupAggregationGuest`
    // directly) and get `main` from std.start — exporting it here too would be a symbol collision.
