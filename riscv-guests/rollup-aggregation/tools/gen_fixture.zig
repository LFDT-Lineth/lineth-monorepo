...
//! package's own `rollup_aggregation_ssz.encodeInput` produced from `support.sampleInput` — the
//! same sample the tests use. Invoked by this guest's own Makefile (`require-input`) to
//! regenerate `make exec`/`debug`'s default input on every run; never checked into git.
