...
schema-valid `RollupOutput` SSZ envelope (schema id `0x1803`), entirely by **echo or sentinel**:
...
| `public_inputs.l2_l1_roots` | exactly one element: **sentinel** |
| `public_inputs.filtered_addresses` | concatenation of every exec proof's `filtered_addresses`, input order (no dedup) |
