...
/// A readable, self-contained `RollupProofPrivateInput`: two conflations, two l2-execution proofs
/// sharing one `program_vk` (so `dedupSortedProgramVks` has something to actually collapse). No
/// external fixture — every field value is visible right here.
fn sampleInput(alloc: std.mem.Allocator) !rollup_ssz.RollupProofPrivateInput {
    ...
    const proof1: rollup_ssz.VerifiableL2ExecutionProof = .{
        .program_vk = repeat32(0xaa), // same VK as proof0 — exercises the dedup below
        .proof = .{
            .public_inputs = .{
...
// ── Echo semantics ────────────────────────────────────────────────────────────────────────────
// `sampleInput` has 2 l2_execution_proofs, both carrying the SAME program_vk — so
