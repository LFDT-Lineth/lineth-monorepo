...
test "run: copied fields echo the mapped source (first/last l2_execution_proofs, input)" {
    ...
    try std.testing.expectEqual(@as(usize, 4), pi.filtered_addresses.len);
    try std.testing.expectEqualSlices(u8, &support.PROOF0_FILTERED_ADDRESS_0, &pi.filtered_addresses[0]);
    try std.testing.expectEqualSlices(u8, &support.PROOF0_FILTERED_ADDRESS_1, &pi.filtered_addresses[1]);
    try std.testing.expectEqualSlices(u8, &support.PROOF1_FILTERED_ADDRESS_0, &pi.filtered_addresses[2]);
    try std.testing.expectEqualSlices(u8, &support.PROOF1_FILTERED_ADDRESS_1, &pi.filtered_addresses[3]);
}
...
test "run: sentinel fields and public-input list fields have their defined values" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const alloc = arena.allocator();
    ...
    try std.testing.expectEqual(@as(usize, 1), pi.l2_l1_roots.len);
    try std.testing.expectEqualSlices(u8, &rollup.L2_L1_ROOTS_ELEMENT, &pi.l2_l1_roots[0]);
