...
async function check() {
  ...
  for (const rel of expectedPartials.keys()) {
    if (!onDisk.includes(rel)) failures.push(`partial: missing _generated/postman/${rel}`);
  }
  for (const rel of onDisk) {
    if (!expectedPartials.has(rel)) {
      failures.push(`partial: unexpected _generated/postman/${rel} (not produced by current sources)`);
    }
  }
  for (const [rel, expected] of expectedPartials) {
    ...
    if (fs.readFileSync(file, "utf8") !== expected) {
      failures.push(`partial: _generated/postman/${rel} is out of date (run pnpm run generate).`);
    }
  }
...
check()
  .then((failures) => {
    if (failures.length) {
      console.error("Postman options drift/completeness check failed:");
      for (const f of failures) console.error(`- ${f}`);
      process.exit(1);
    }
    console.log("Postman options check passed (committed output matches source; wrapper completeness OK).");
  })
  .catch((err) => {
