/**
 * Linea Postman env-var config MDX renderer.
 * Extraction is done by parse-postman.js; this file turns the extract into MDX partials + wrapper.
 */

const path = require("node:path");
const prettier = require("prettier");

const { extract, SECTION_META } = require("./parse-postman");
...
function renderSectionPartial(section, keys) {
  const lines = [];
  ...
  for (const k of keys) {
    const row = [
      "`" + escapeInlineCode(k.envVar || k.key) + "`",
      escapeCell(k.description),
      renderDefault(k.default),
      renderType(k.type),
      escapeCell(k.requiredLabel || k.allowedRequired || (k.required ? "required" : "optional")),
    ];
    lines.push("| " + row.join(" | ") + " |");
    rowCount++;
  }
...
function renderPartials(manifest) {
  ...
  for (const s of SECTION_META) {
    ...
    sectionKeys.sort((a, b) => (a.envVar || a.key).localeCompare(b.envVar || b.key));
    const { markdown, rowCount: n } = renderSectionPartial(s, sectionKeys);
    rowCount += n;
    partials.push({
      section: s.id,
...
function renderStarterWrapper(partials) {
  ...
  lines.push(
    "{/* Human-owned wrapper. Automation only updates `_generated/postman/` partials. " +
      "Seeded once by scripts/postman-options; place new partial imports when sections appear. */}",
  );
  lines.push("");

  lines.push("import Provenance from './_generated/postman/provenance.mdx';");
  for (const part of partials) {
    lines.push(`import ${part.componentName} from './_generated/postman/${part.relPath}';`);
  }
  lines.push("");
 ...
 * Publish-only provenance.mdx is allowed under _generated/postman/.
 */
function checkCompleteness(wrapperMarkdown, partialRelPaths) {
  const failures = [];
  const importRe = /import\s+([A-Za-z_][\w]*)\s+from\s+['"]\.\/_generated\/postman\/([^'"]+)['"]/g;
  ...
  for (const rel of expected) {
    if (!imported.has(rel)) {
      failures.push(
        `generated partial _generated/postman/${rel} is not imported by the wrapper (place the import and <Component />)`,
      );
    } else {
      const name = imported.get(rel);
      const usage = new RegExp(`<${name}\\s*/>`);
      if (!usage.test(wrapperMarkdown)) {
        failures.push(`wrapper imports ${name} from _generated/postman/${rel} but never renders <${name} />`);
      }
    }
  }
  for (const rel of imported.keys()) {
    ...
    failures.push(`wrapper imports _generated/postman/${rel} but that partial was not generated`);
  }
  return failures;
}
...
async function formatWith(text, ext, toolRoot) {
  const filepath = path.join(toolRoot, `postman-options-output.${ext}`);
...
async function build({ monorepoRoot, toolRoot } = {}) {
  ...
  const { manifest, report } = extract(mono);
  return buildFromExtract({ manifest, report, toolRoot: root });
}

async function buildFromExtract({ manifest, report, toolRoot } = {}) {
  const root = toolRoot || __dirname;
  const { partials, rowCount } = renderPartials(manifest);

  if (rowCount !== manifest.counts.rendered) {
    throw new Error(
      `Count mismatch: rendered ${rowCount} rows but manifest counts ${manifest.counts.rendered} documentable env vars.`,
    );
  }
  ...
  for (const p of formattedPartialsList) {
    if (!isNeutralPartial(p.markdown)) {
      throw new Error(`Partial _generated/postman/${p.relPath} is not neutral (front matter, import, or custom JSX).`);
    }
  }
