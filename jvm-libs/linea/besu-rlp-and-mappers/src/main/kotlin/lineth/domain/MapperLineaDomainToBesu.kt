...
import lineth.domain.MapperLineaDomainToBesu.mapToBesu
...
fun lineth.domain.Transaction.toBesu(): Transaction = mapToBesu(this)
...
object MapperLineaDomainToBesu {
  ...
  internal fun signature(tx: lineth.domain.Transaction, recId: Byte): SECPSignature {
  ...
  fun getRecIdAndChainId(tx: lineth.domain.Transaction): Pair<Byte, BigInteger?> {
  ...
  fun mapToBesu(blockNumber: ULong, txIndex: Int, tx: lineth.domain.Transaction): Transaction {
  ...
  fun mapToBesu(tx: lineth.domain.Transaction): Transaction {
