...
class Web3jEthApiClientWithRetries(
  ...
  private fun stopRetryOnFinalizedSafeTags(th: Throwable): Boolean {
    return if (th.cause is lineth.error.JsonRpcErrorResponseException) {
      val rpcError = th.cause as lineth.error.JsonRpcErrorResponseException
  ...
  val stopOnExecutionRevertedPredicate: Predicate<Throwable> = Predicate { th ->
    ...
    if (th.cause is lineth.error.JsonRpcErrorResponseException) {
      val rpcError = th.cause as lineth.error.JsonRpcErrorResponseException
