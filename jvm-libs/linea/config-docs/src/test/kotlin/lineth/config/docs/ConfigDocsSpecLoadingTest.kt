...
class ConfigDocsSpecLoadingTest {
  ...
  fun `loads a Kotlin object spec by fully-qualified name`() {
    val spec = loadSpec("lineth.config.docs.ConfigDocsSpecLoadingTest\$SampleSpec")
  ...
  fun `fails when the class cannot be found`() {
    assertThatThrownBy { loadSpec("lineth.config.docs.NoSuchSpec") }
