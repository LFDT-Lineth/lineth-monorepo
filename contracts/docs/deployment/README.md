...
**Shared L1 base (Lineth Rollup & Validium):** `INITIAL_L2_BLOCK_HASH`, `INITIAL_L2_BLOCK_NUMBER`, `L2_GENESIS_TIMESTAMP` — common to both products via shared contract base. `INITIAL_L2_BLOCK_HASH` is the initial L2 block hash, passed as `initialBlockHash` on-chain.

**Product-specific:** `LINETH_ROLLUP_OPERATORS`, `LINETH_ROLLUP_RATE_LIMIT_*`, `LINETH_ROLLUP_VERIFIER_KEYS` (optional on fresh deploy; seeds guest-program keys at `initialize`, with `SET_VERIFIER_KEY_ROLE` / `UNSET_VERIFIER_KEY_ROLE` defaulting to the security council via `LINETH_ROLLUP_V8_ROLES`), `VALIDIUM_OPERATORS`, `VALIDIUM_RATE_LIMIT_*`, `VALIDIUM_VERIFIER_KEYS` (optional on fresh deploy; same role defaults via `VALIDIUM_ROLES`)
