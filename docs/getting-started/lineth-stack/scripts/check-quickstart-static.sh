...
check_reuse_guardrails() {
  ...
  if grep -q 'corepack prepare pnpm@12.6.0 --activate' "$ensure_demo_erc20_sh" \
  ...
  if grep -q 'corepack prepare pnpm@12.6.0 --activate' "$traffic_account_sh"; then
...
check_pinned_utility_images_and_docs() {
  ...
  if grep -q 'lineth-foundry-home:' "$compose" \
    ...
    && grep -q 'corepack prepare pnpm@12.6.0' "$deploy_contracts" \
