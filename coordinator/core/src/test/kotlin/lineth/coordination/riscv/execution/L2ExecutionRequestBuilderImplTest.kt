...
class L2ExecutionRequestBuilderImplTest {
  ...
  private val payloadRequests = mutableListOf<BlockNumberAndHash>()
  ...
  fun `matches payload and witness by hash and preserves block order despite completion order`() {
    ...
    val payloads = blocks.map {
      it.toBesu().toExecutionPayload(byteArrayOf(0xc0.toByte())).copy(slotNumber = it.number + 10UL)
    }
    ...
    assertThat(payloadRequests).containsExactlyElementsOf(blocks.map { BlockNumberAndHash(it.number, it.hash) })
  ...
  fun `missing witness fails the request`() {
    ...
    for (index in blocks.indices) {
      payloadFutures[index].complete(blocks[index].toBesu().toExecutionPayload(ByteArray(0)))
  ...
  fun `unavailable payload fails the request`() {
    val future = builder.build(conflation)
    for (index in blocks.indices) {
      payloadFutures[index].completeExceptionally(IllegalStateException("BAL unavailable"))
      witnessFutures[index].complete(ExecutionWitness(emptyList(), emptyList(), emptyList()))
    }
