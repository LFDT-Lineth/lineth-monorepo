...
data class RollupProofRequestV1(
  ...
  override val endBlockTimestamp: Instant
    get() = l2Executions.last().endBlockTimestamp
  override val transactionsCount: Long
    get() = l2Executions.sumOf { it.transactionsCount }
  override val totalGasUsed: Long
    get() = l2Executions.sumOf { it.totalGasUsed }
 ...
 *
 * Domain twin of `lineth.coordinator.clients.prover.RollupPublicInputsDto`. Kept here (rather than reusing the
 ...
 *
 * Mirrors `lineth.coordinator.clients.prover.RollupProofResponseDto`: the DTO's `String` (hex) fields are
